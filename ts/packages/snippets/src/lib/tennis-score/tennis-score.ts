export enum Score { Love = '0', Fifteen = '15', Thirty = '30', Forty = '40', Advantage = 'Adv.', Game = 'Game' }
...
export class TennisScore {
  ...
  player1Scores(): TennisScore {
    const {first, second} = this.computeScore(this.player1Score, this.player2Score);
  ...
  private computeScore(winningPointScore: Score, opponentScore: Score): ({ first: Score, second: Score }) {
  ...
  player2Scores(): TennisScore {
    const {first, second} = this.computeScore(this.player2Score, this.player1Score);
