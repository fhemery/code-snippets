...
describe('createUserService', () => {
  ...
  describe('when using a stub', function () {
    ...
    it('should return the id send by the repository', () => {
      service = new CreateUserService(new StubUserRepository(), new UserValidationService(), {} as NotificationService);
  ...
  describe('when using a spy', function () {
    it('should check validation service is called', () => {
      ...
      service = new CreateUserService(new FakeUserRepository(), validationService, {} as NotificationService);
      const spy = jest.spyOn(validationService, 'sendActivationLinkToUser');
  ...
  describe('when using a mock', function () {
    it('should check validation service is called with right parameter', () => {
      const validationService = new UserValidationService();
      service = new CreateUserService(new FakeUserRepository(), validationService, {} as NotificationService);
      const spy = jest.spyOn(validationService, 'sendActivationLinkToUser').mockImplementation((id) => {
        if (id !=='1') {
  ...
  describe('when using a fake', function () {
    it('should return the id send by the repository', () => {
      ...
      service = new CreateUserService(fakeUserRepository, new UserValidationService(), {} as NotificationService);
